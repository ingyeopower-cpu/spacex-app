...
### 2. 로컬에서 웹 버전 실행하기 (개발용)
PC 터미널에서 웹 버전을 바로 실행하여 모바일 브라우저로 접속할 수 있습니다.
- PC와 모바일 기기를 **같은 Wi-Fi**에 연결합니다.
- PC 터미널에서 `npx expo start --web`을 입력합니다.
- 터미널에 표시된 Local URL(예: `http://192.168.x.x:8081`)을 모바일 브라우저(Chrome, Safari 등) 주소창에 직접 입력합니다.
